...
- **Migración completa (2026-07-14):** `index.html`, `publications.html`, `videos.html`, `presentaciones/index.html` y el CV (repo aparte) están todos en el sistema "Mono claro / Field Note". Ya no dependen del `style.css` anterior. `style.css` quedó huérfano (se puede borrar tras verificar que nada más lo use).
